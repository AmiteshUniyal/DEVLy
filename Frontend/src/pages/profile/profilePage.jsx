...
const ProfilePage = () => {
	...
	const handleImgSubmit = async(e) => {
		...
		try {
		  ...
		  const res = await axiosInstance.post("/user/update", { profileImg : profileImgBase64, coverImg : coverImgBase64 }, { withCredentials: true });
		  ...
		  console.error("Failed to update images", err.response?.data?.error || err.message);
		} 
		finally {
		  setIsPending(false);
		}
					...
					{!isLoading && userProfile && (
						<div>
							...
							<div className='relative group'>
								...
								<div className='absolute -bottom-16 left-3'>
									<div className='w-32 rounded-full relative group'>
										<img
											src={profileImgPrev || userProfile?.profileImg || "/avatar-placeholder.png"}
							...
							<div className='flex flex-col gap-4 mt-14 px-4'>
								...
								<div className='flex gap-4 flex-wrap'>
									...
									<div className='flex gap-2 items-center'>
										...
										<span className='text-sm text-slate-400'>Joined {joinedDate}</span>
									</div>
								</div>
							...
							<div className="relative w-full border-b border-gray-500 mt-4">
								...
								<div className="absolute bottom-0 h-1 bg-blue-500 transition-all duration-300"
									style={{
										width: "50%",
					...
					{isMyProfile || isFollow ? (
						postType === "posts" || postType === "likes" ? (
							<div className="w-full overflow-hidden">
								<Posts type={postType} username={userProfile.username} Id={userProfile._id}/>
							</div>
						) : (
